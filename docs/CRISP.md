...
v1.2.1 的正常 AI 答案不再由程序附加评价邀请、评分链接或评价按钮。用专用访客继续发送普通“是/否/👍/👎”，应按咨询及既有业务规则处理，不因旧待评价记录计分或自动转人工。历史统计保留，不要为验证清空客户记录。

本项目发送者使用中性的“在线客服”，不在名称或正文额外追加“机器人/AI 回复”标签，不请求可选 `automated` 徽标。内部仍在 POST 前持久登记按网站/会话绑定的 fingerprint，并兼容旧 automated 回流；不能凭昵称判定真人，也不能为改变外观关闭回流保护或擅改其他机器人的设置。平台是否仍显示徽标必须通过真实 SDK 核验，未完成前不承诺已隐藏。访客直接询问身份时应如实说明是自动客服，不伪造真人身份。

1. A/B 各问一个虚构知识问题，确认答案和上下文不混用；A 再发一张不含隐私的图片并追问。
2. A 输入“人工”：默认应看到原生“召唤人工客服 / 继续咨询”选择卡片；不点击，A 再问普通问题仍得到自动回复，B 不变。已有合法自定义按钮标题仍按配置显示。
...
5. 用 `7 → 5` 只调整专用 A 的恢复时间做短秒数验证，不修改用户全局默认 3600 或已有自定义值。真人新的公开回复按现有默认策略更新 A 截止时间，不能仍假定沿用测试前的 10 秒；以回读为准。访客消息不延长，0 表示需管理员明确恢复。测试后保留/恢复原定策略，不影响 B。
6. 仅在获准的隔离实例或维护窗口测试 `crispai disable`：所有自动出站静默，真人继续工作；重新启用保留各会话人工状态、不补答历史。操作前记录总开关和欢迎原值，操作后恢复，不在普通客户接待中随意切换全局。

主备验收与 Crisp 收发分层：先验证正常原窗口往返，再仅在获准测试条件下让主接口发生受控故障，检查 `3 → 11` 的脱敏切换事实及同一访客最终回复。协议服务故障注入不等于多家真实账号验收；没有真实备用应如实记录，不复制主 Key 冒称独立配额。不以模型的内容安全拒绝作为绕过测试，也不凭 REST 发送成功代替 SDK 收件。

原生控件采用 Crisp `picker` 的 id/text/choices/value/label/selected。项目动作不写入官方只支持 link/frame 的 `choice.action.type`；发送前登记自有 fingerprint，使用官方可选 `user` 的网站类型及中性昵称，不伪造 operator 身份。按钮有效性还受本机会话、期限、代次和消费记录限制，手打按钮文字不会接管。依据：[Crisp REST 消息契约](https://docs.crisp.chat/references/rest-api/v1/)。

人工识别不要求 Hook 一定带 `automated:false`：官方公开操作者消息可省略该字段，程序同时核对其操作者 ID，并优先排除本项目自有指纹和明确自动消息。网站级 REST 发送不能冒称后台真人操作，单凭设置 false 也不能证明平台回流保留了它；身份不明时有限回查并作废旧答案，不根据昵称猜身份。依据：[官方 Message Received 示例](https://docs.crisp.chat/references/web-hooks/v1/#message-received)。

## 4. 欢迎消息与自动展开是两项配置

`crispai → 9` 管理欢迎。默认欢迎启用、首条访客消息触发，自动展开关闭；首条消息模式无需修改网页。欢迎每会话持久去重，人工期间或总开关关闭不发送。

`9 → 3` 关闭的是 `menu.welcome.enabled`，不是客服总开关。关闭后应看动作的中文结果并用 `9 → 1` 回读欢迎状态；总开关可以继续启用。旧版关闭动作曾直接显示 runtime 的 `enabled:true`，该字段不能证明欢迎仍开。v1.2.1 普通菜单按实际业务对象呈现，结构化脚本接口另行保留。
