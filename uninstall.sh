...
info "卸载前创建不含密钥的自动备份"
"$BACKUP_SCRIPT" --deploy-dir "$DEPLOY_DIR" --output "$BACKUP_FILE"
...
if [[ "$MODE" == purge ]]; then
  ...
  printf '保留内容：部署目录外的不含密钥自动备份：%s\n' "$BACKUP_FILE"
  printf '恢复方式：重新克隆项目并运行 install.sh，重新输入 Provider/Crisp 凭据，再用 restore.sh 导入上述备份。\n'
  printf '注意：完整清理已按确认删除 AnythingLLM 与 n8n 运行数据，便携备份不能恢复这些历史运行数据。\n'
  exit 0
fi
...
printf '自动备份：%s（不含 Token、Secret 或 .env）\n' "$BACKUP_FILE"
