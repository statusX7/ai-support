...
provider_candidate() {
  ...
  if [[ -n "$input" ]]; then
    ...
    jq -e 'type == "object" and (.provider | type == "object")' "$input" >/dev/null || return 1
    base=$(jq -r --arg value "$base" '.provider.base_url // $value' "$input")
    model=$(jq -r --arg value "$model" '.provider.model // $value' "$input")
    mode=$(jq -r --arg value "$mode" '.provider.api_mode // $value' "$input")
    if jq -e '(.api_key // "") | length > 0' "$input" >/dev/null; then key=$(jq -r '.api_key' "$input"); fi
    if jq -e '.provider | has("custom_headers") or has("remove_header")' "$input" >/dev/null; then
      headers=$(printf '%s' "$headers" | jq -c --slurpfile candidate "$input" '
  ...
  printf '%s' "$headers" | jq -e 'type == "object" and length <= 32 and all(to_entries[];
  ...
  printf '%s' "$key" | jq -Rs --arg base "$base" --arg model "$model" --arg mode "$mode" --slurpfile headers "$headers_file" \
...
provider_request() {
  ...
  base=$(jq -r '.provider.base_url' "$candidate")
  ...
  jq -r '"header = " + ("Authorization: Bearer " + .api_key | @json),
...
provider_probe_candidate() {
  ...
  model=$(jq -r '.provider.model' "$candidate")
  validate_model_identifier "$model" || { configuration_error '请先选择或填写模型'; return 1; }
  mode=$(jq -r '.provider.api_mode' "$candidate")
  ...
  for endpoint in chat/completions responses; do
    ...
    if [[ "$endpoint" == responses ]]; then
      payload=$(jq -cn --arg model "$model" --arg image "$image" --argjson vision "$vision" '{model:$model,store:false,max_output_tokens:64,input:[{role:"user",content:([{type:"input_text",text:"只回复：测试正常"}]+if $vision then [{type:"input_image",image_url:$image}] else [] end)}]}')
    else
      payload=$(jq -cn --arg model "$model" --arg image "$image" --argjson vision "$vision" '{model:$model,messages:[{role:"user",content:(if $vision then [{type:"text",text:"请描述图片颜色。"},{type:"image_url",image_url:{url:$image}}] else "只回复：测试正常" end)}]}')
    fi
    ...
    [[ "$status" == 2?? ]] || continue
    if [[ "$endpoint" == responses ]]; then
      if jq -e '(.error == null or .error == false) and (.status != "failed") and
    ...
    elif jq -e '(.error == null or .error == false) and (.choices[0].message.content | type == "string" and test("\\S"))' "$response" >/dev/null 2>&1; then chat=true; fi
  done
  rm -f -- "$response"
  if [[ "$mode" == auto ]]; then
  ...
  jq -cn --arg mode "$mode" --argjson chat "$chat" --argjson responses "$responses" --argjson vision "$vision" \
...
provider_apply() (
  ...
  mode=$(jq -r '.api_mode' <<< "$result")
  config_candidate=$(mktemp "${deploy_dir}/config/provider.yaml.tmp.XXXXXX")
  jq --arg mode "$mode" '.provider.api_mode=$mode' "$candidate" > "$config_candidate"
  ...
  value=$(jq -r '.provider.base_url' "$candidate")
  ...
  value=$(jq -r '.api_key' "$candidate"); env_set "$env_candidate" AI_API_KEY "$value"
  value=$(jq -r '.provider.model' "$candidate"); env_set "$env_candidate" AI_MODEL "$value"
  value=$(jq -c '.provider.custom_headers' "$candidate"); env_set "$env_candidate" AI_CUSTOM_HEADERS_JSON "$value"
  ...
  config_candidate=$(mktemp "${deploy_dir}/config/provider.yaml.tmp.XXXXXX")
  jq --arg base "$runtime_base" --argjson result "$result" --argjson vision "$vision" '{schema_version:2,provider:(.provider | .base_url=$base | .api_key_env="AI_API_KEY" | .custom_header_names=(.custom_headers | keys) | del(.custom_headers) | .capabilities=$result.capabilities | .capabilities.vision=$vision)}' "$candidate" > "$config_candidate"
  ...
  if ! docker_compose "$deploy_dir" config --quiet || ! docker_compose "$deploy_dir" up -d provider-adapter anythingllm n8n >&2 || ! wait_for_local_health "$deploy_dir" >&2 || ! provider_runtime_test "$deploy_dir"; then
    configuration_revision "$deploy_dir"
    cp -p -- "${history}/previous.env" "${deploy_dir}/.env"
    cp -p -- "${history}/previous.yaml" "${deploy_dir}/config/provider.yaml"
    if docker_compose "$deploy_dir" up -d provider-adapter anythingllm n8n >&2 && wait_for_local_health "$deploy_dir" >&2 && provider_runtime_test "$deploy_dir" >/dev/null; then
...
provider_main() {
  ...
  case "$action" in
    get|show) jq '.provider | .key_status="已保存" | .custom_header_names=(.custom_headers | keys) | del(.custom_headers)' "$candidate" ;;
    models)
      ...
      if [[ "$status" != 2?? ]] || ! jq -e '.data | type == "array" and any(.[]; .id | type == "string" and length > 0)' "$response" >/dev/null 2>&1; then
        rm -f -- "$candidate" "$response"; configuration_error "模型列表不可用（HTTP ${status}），可以手动填写模型"; return 2
      fi
      digest=$(sha256sum "$candidate" | awk '{print $1}')
      jq --arg digest "$digest" '{cache_revision:$digest,models:([.data[]?.id | select(type == "string" and length > 0)] | unique | .[0:1000])}' "$response" > "${deploy_dir}/data/provider-models.json"
      chmod 600 "${deploy_dir}/data/provider-models.json"
      jq '.' "${deploy_dir}/data/provider-models.json"; rm -f -- "$response" ;;
