...
控制状态 `data/runtime/session-SHA256(website+NUL+session).json` 使用短 mkdir 锁、fsync/原子写。每会话普通消息队列有序，不在长 LLM 请求期间持锁，不同会话独立。全局开关来自资料投影中的 runtime.enabled，不是共享人工状态。无限人工不受已完成任务缓存淘汰影响。生成投影损坏、资料应用中或知识映射代次不一致时，不继续用半份原文发送普通回复；已接收问题持久延后，资料成功发布后按最新代次恢复一次。期间发生真人接管或总开关关闭仍永久取消对应问题。人工事件始终走持久控制路径。
