...
这是服务器调用模型的 API 地址，从供应商的 API 接入文档取得，不能填网页版聊天地址，也不要填完整的 `/chat/completions` 或 `/responses` 请求路径。程序会把地址规范为版本基址：例如 `https://api.example.com/api` 保存为 `https://api.example.com/api/v1`，已经是 `https://api.example.com/api/v1` 时不会再次追加；其他合法代理前缀同样保留并只补一个末尾 `/v1`。地址通常不是密钥，但私有网关地址也不要公开。

验证：第 3 项先读取模型列表，再按 Chat Completions 或 Responses 对所选模型取得有效正文；模型列表成功本身不代表推理成功。装好后用 `3 → 9` 选择主接口，从实际 Provider adapter 路径显式测试。填错用 `3 → 2 → 2`；更换整家供应商时在 `3 → 2` 一起修改地址、Key、协议和模型，最后选编辑页 `9` 验证并保存。
...
验证：安装的少量能力请求须得到有效正文；401/403 要先核对权限，模型列表不能单独证明推理可用。填错用 `3 → 2 → 3`，编辑页 `9` 验证保存；`3 → 9` 可再明确测试指定接口。

### 3/10：模型

这是该 Key 获准调用的模型 ID，来自供应商模型列表或文档。优先按实际列表数字选择；虚构格式例子：`demo-chat-model`。模型 ID 通常不是秘密。列表可搜索/翻页；供应商不提供列表时允许手填，不能猜一个名字当成验证成功。

验证：向导检查 Chat Completions / Responses 的实际回答，装好后 `3 → 9` 选择接口，再选文本或图片测试。填错用 `3 → 2 → 4` 获取/选择主接口模型，或 `3 → 2 → 5` 手填，最后选编辑页 `9` 验证保存。
...
十项初始化只填写主接口，不要求备用或多套 Key。装好后 `3 → 3` 可逐个添加可信备用，`3 → 1` 查看顺序；全池最多 21 项，停用项和待补凭据项也占容量。没有备用合法，但主接口故障时没有第二家可接替。不要把同一账户下多个模型当成多套独立配额。
...
新装默认启用客服、欢迎语和人工确认按钮，人工自动恢复为 3600 秒，页面自动展开关闭。`0` 秒表示保持人工直至管理员恢复，不是立即恢复。升级保留已有合法显式秒数；如需一小时，从 `crispai → 7` 设置 3600，不必重装。v1.2.1 另外强制停用旧自动评价邀请，不删除历史统计。普通“是/否/👍/👎”仍按普通咨询处理，不作为自动评分或转人工指令。
...
- 将主接口建立为受管接口池，分开保存非秘密配置、受限上游凭据和内部 adapter 认证；重复初始化不从旧兼容字段覆盖已有池；
...
默认部署目录是 `/opt/crisp-ai`。安装过程可因首次镜像、Embedding 模型下载和知识索引持续较长时间。Ctrl+C、EOF 或网络失败后，向导进度和已生成的有效密钥保存在受限部署状态中。再次进入处于 `collecting` 的向导时会显示上次待填步骤，并要求数字选择：`1 继续上次 / 2 重新开始 / 0 取消`；不会无提示决定替你覆盖或继续。

选择继续时从记录的步骤恢复，已经确认的前置输入不重复询问。选择重新开始时只清理属于本次向导、位于受管工作目录、身份和权限均通过检查的临时知识草稿，然后重置十项收集状态；不会删除 `.env`、已部署配置、Prompt、正式知识库、内部密钥、数据库或会话状态。遇到无法安全确认归属的草稿会保留并报错，而不是扩大删除范围。
...
| --- | --- |
| 未安装 | 下载当前稳定正式包并直接初始化。 |
| 初始化中断（`collecting`） | 固定到记录的原版本，显示下一待填步骤，再数字选择继续、重新开始或取消；重新开始只清理受限向导草稿。 |
...
退出码 `0` 表示所选操作完成；`2` 可表示用户明确取消，或本地安装成功但外部仍待接入，需结合屏幕文字与状态判断；`130` 是受控中断。在线引导、`doctor`、`logs` 与资料命令把参数错误定义为 `64`；部分兼容的更新、卸载脚本仍以 `1` 报参数或阶段错误，应同时阅读同一屏的中文原因和对应命令 `--help`，不要只凭一个数字重装。`local-ready` 不会被冒充为已经接待客户，也无需因此重装本地组件。
...
受管模式会先用本项目锁定的 Caddy 镜像验证候选配置，验证通过后才启动或替换本项目 Caddy；随后从容器内回读实际挂载文件摘要。若候选无法启动或回读不一致，程序尝试恢复最近一份已验证配置并仍把本次操作报告为失败，不会拿“容器进程已出现”冒充新配置生效。由受管 HTTPS 切换到已有外部反代时，只退役本实例的受管 Caddy；切回受管模式则重新校验并对账，不修改外部站点配置。
...
真实 Crisp、真实第三方模型、DNS/TLS 和站点页面权限属于部署者的外部资源。普通用户缺少这些条件时，安装结果必须如实标记 `External Validation Pending`，不能称为已经接待。v1.2.1 维护者发布另有硬门槛：完整冻结候选先通过已授权 TARGET-A 验收，才可创建 tag、推送或上传发布资产；协议测试不能冒充真实客户往返。
...
前一条只校验，后一条明确应用业务资料及接口池原文；菜单等价入口是 `16 → 6 → 1`。有接口候选变化时，应用可能执行少量验证并计费；只读检查、列表和空闲状态不会自动轮询付费。普通 Prompt/知识/欢迎菜单只提交自己的业务修改，不顺带应用另一个尚未确认的接口池候选。

未编辑完的坏 JSON、空 Prompt 或超限文件不会替代上一有效资料。若业务已经应用、接口池验证或回读失败，界面会分别说明：业务新值保留、接口池沿用上一有效配置，整体不能算全部完成。原文没变而外部 Prompt/受管索引关系偏离时，用 `16 → 6 → 2` 重新同步并回读；它不是全量重建或远端正文逐字审计。不要手改 `config/materials-applied.json` 或 `config/provider-pool-applied.json`；应用失败、`applying`、导入草稿和索引重建边界见 [配置说明](CONFIG.md)。
...
主备切换默认一个逻辑问题共用 90 秒、最多 21 次上游调用，单次 20 秒、连接 5 秒；图片解析与最终回答不各拿一份预算。可在 `3 → 10` 调整受限策略。备用可能接收当前 Prompt、必要知识、公开历史或图片，并增加费用；只添加自己信任且获准使用的服务。切换不会修改 workspace、重建索引或把访客转人工，内容安全拒绝不会尝试另一家绕过。
