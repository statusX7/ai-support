...
需要更新 Prompt/知识时进入 `applying`，使旧计算结果失效，同步完成并真实回读后发布新 revision。期间已经收到的普通访客问题会先持久延后，不使用半份配置；应用成功后绑定最新 revision，重新规划并只回复一次。期间若真人接管该会话或管理员明确关闭总开关，问题按该业务状态取消且不会补答。人工控制始终独立处理。菜单等价入口为 `16 → 6 → 1`，状态用 `16 → 8`。底层复用 `scripts/materials.sh` 的 `status | validate | initialize | apply`；内部 `apply-business` 供单项业务维护使用，不顺带发布尚未确认的接口池编辑。`initialize` 用于安装/升级建立首份投影。
...
程序拥有的异常回复采用简短的具体情况/图片文字澄清，不发系统忙或例行稍后重试话术，不声称后台正在处理。Provider 虽返回 200、整段正文却只是系统维护、模型繁忙或稍后再试时，按 `invalid_response` 在同一共享预算内切换备用；安全拒答、带具体处理步骤的说明和业务可用性事实不切换。旧 `handoff.failure_message` 仅为兼容保留，不再控制自动异常出站；尚未登记发送的旧模型缓存会规范化，`unknown/sending` 则始终先按原正文和 fingerprint 对账，不能改写可能已送达的历史。普通用户正文、知识原文、关键词/菜单/欢迎固定回复和已送历史不作全局替换。维护日志及自检仍如实显示故障，不将澄清计为有效模型答案。
...
客服检索只使用当前问题，系统护栏、内部认证信封和完整聊天历史不会混入 Embedding 输入；原 Prompt、已选知识块与本会话历史在生成阶段分别传入。明确的 FAQ 原问或足够具体且保留区分实体的原文短语可先由同代词法索引命中；strong 命中不再被旧向量排名挤掉。其余问法继续使用向量检索。对少量代码内固定、与业务正文无关的紧义中文概念，受管词法索引可产生 `synonym_overlap` 候选；它不是 strong 命中，必须先执行向量检索，仅在向量没有有效来源时保守补充。近似分支要求来源问题不超过 4096 字节，主题概念、实体限定、孤立字符与编号顺序一致，并拒绝引用/括号、复合改问、元指令、混合主题和危险分隔结构；泛问或缺少唯一实体时继续走纯向量。词法与向量结果按来源/正文去重，普通词语重叠不能冒充语义命中。workspace 的实际 topN、similarityThreshold 和 vectorSearchMode 仍决定向量结果，运行时合并后最多接受 20 块，不会为了掩盖漏检而自动降低阈值。已选片段须对应当前启用映射，完整传递而不在预处理时截断；遇到窗口不足则准确失败，不将缺失的片段解释为不存在的业务事实。来源非空也不是答案正确证明，需用原问和同义问题分别核对片段包含明确事实且最终答案采用了它。
