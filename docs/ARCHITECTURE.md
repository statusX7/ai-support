...
Crisp POST 已交出字节但回执未知时，原正文、计划和 fingerprint 保持绑定。重试前先查会话消息列表；列表可能短暂滞后，因此 10 秒后还必须查询精确消息端点。列表成功后，精确端点的 404 只算一次“未找到”；其它鉴权、限流、服务或网络失败仍是未知。仅在两次至少间隔 5 秒的明确阴性结果后，才允许沿用同一 fingerprint 再发送；查询失败只继续对账。首次发送和最多两次恢复发送合计不超过三次，每轮恢复都重新取得双重阴性证据。配置换代时同样先收敛旧 fingerprint，避免按新规则产生第二条答案。人工确认通知属于受限控制消息：会话已正确进入人工模式不会让它自行失效，但总开关关闭或会话代次变化仍会取消。三次 POST 结果均未知时在五分钟窗口内只对账，超时记录明确送达故障；跨系统仍不宣称绝对 exactly-once。
