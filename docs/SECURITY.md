...
默认安全卸载会先创建不含密钥的迁移备份，确认 Docker 资源已移除后再删除程序、Compose 和脚本，同时保留 `.env`、业务配置、知识文件、运行数据、备份和日志。保留 `.env` 是恢复 PostgreSQL 数据与 n8n 加密内容所必需的；它保持 `0600`，但部署目录仍必须按含真实凭据的敏感资产保护。Docker 不可用或停止失败时，卸载必须在删除程序和数据前中止。
