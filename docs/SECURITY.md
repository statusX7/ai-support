...
- 中断时的十项向导状态也为 `0600`；本地应用配置成功后立即删除，避免长期保存第二份 Provider/Crisp 凭据。
...
`.env` 不会被 `source` 为 Shell 代码。特殊字符使用 Compose dotenv 兼容格式原子写入，并在发布前通过真实 Compose/容器往返检查。Provider 的 Bearer Token 不跟随 HTTP 重定向；除本机回环测试服务外，Provider 必须使用 HTTPS。
...
直接调用 `--purge` 会删除整个部署目录，必须先输入 `y` 或 `yes`，再精确输入 `PURGE`；`--yes` 不得绕过任一次确认。中文管理菜单改用两次数字 `1` 确认，删除边界相同。完整清理始终在部署目录同级生成不含密钥的最终迁移备份，不提供跳过备份或绕过 Docker 检查的选项。该备份不包含 `.env`、数据库或 AnythingLLM 运行数据，不能代替需要原密钥的数据级恢复点。
...
- 自动安装 Docker 不改变其资产归属：卸载本项目永远不会卸载 Docker、containerd 或全局软件包，也不会执行全局 prune。
- 受管 Caddy 只公开生产 Webhook 路径；n8n 编辑器、AnythingLLM 和数据库不随自动 HTTPS 暴露。已有 80/443 服务不会被停止或覆盖。
